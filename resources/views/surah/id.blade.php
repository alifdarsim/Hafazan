...
@section('content')
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        ...
        <div id="verse-container"
             class="text-teal-900 mx-auto mt-8 grid grid-cols-1 col-span-2 w-full max-w-6xl gap-2 sm:gap-5 lg:gap-6">
            ...
            @for($v=1; $v<=$chapters[$id-1]->versesCount; $v++)
                <x-verse-container key="{{$id}}:{{$v}}" index="{{$v}}" surah="{!! $chapters[$id-1]->nameSimple !!}"/>
            @endfor
...
@push('scripts')
    ...
    <script type="module" src="{{ Vite::asset('resources/js/view/surah/progress.js') }}"></script>
@endpush
